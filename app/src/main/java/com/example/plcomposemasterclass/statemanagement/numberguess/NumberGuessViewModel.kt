...
class NumberGuessViewModel : ViewModel() {
    private var number = Random.nextInt(from = 1, until = 101)
    ...
    fun onAction(action: NumberGuessAction) {
        when (action) {
            ...
            NumberGuessAction.OnStartNewGameButton -> {
                number = Random.nextInt(from = 1, until = 101)
                attempts =
                    0 // This line is not on the course but is needed to not accumulate attempts
                _state.update {
                    it.copy(
                        numberText = "",
                        guessText = null,
